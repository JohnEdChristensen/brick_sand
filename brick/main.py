                ...
                match particle:
                    ...
                    case particleType.water: # Does not work correctly!
                        if y>0 and y < HEIGHT-1 and x >0 and x < WIDTH-1:
                            ...
                            if (self.grid[x][y-1] == particleType.water or
                             self.grid[x+1][y-1] == particleType.water or
                                self.grid[x-1][y-1] == particleType.water):
                                if self.grid[x-1][y] ==particleType.empty:
                                    newGrid[x-1][y] = particleType.water
                                elif self.grid[x+1][y] ==particleType.empty:
                                    newGrid[x+1][y] = particleType.water
                                else:
                                    newGrid[x][y] = particleType.water
                            else:
